...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    private int defaultTabWidth;//固定每个tab的宽度,默认为屏幕的1/5
    private LinearLayout.LayoutParams default_tabLayoutParams;
    ...
    public void notifyDataSetChanged() {
        ...
        if (tabCount<5&&tabCount>0){
            defaultTabWidth = getResources().getDisplayMetrics().widthPixels / tabCount;
            default_tabLayoutParams = new LinearLayout.LayoutParams(defaultTabWidth, LinearLayout.LayoutParams.MATCH_PARENT);
        }else if (tabCount>5){
            defaultTabWidth = getResources().getDisplayMetrics().widthPixels / 5;
            default_tabLayoutParams = new LinearLayout.LayoutParams(defaultTabWidth, LinearLayout.LayoutParams.MATCH_PARENT);
        }
...
//			v.setBackgroundResource(tabBackgroundResId);
            ...
            if (v instanceof TextView) {
                ...
                mTab = (TextView) v;
                mTab.setTextSize(TypedValue.COMPLEX_UNIT_PX, tabTextSize);
                mTab.setTypeface(tabTypeface, tabTypefaceStyle);
                //tab默认颜色的划分
                if (i == currentPosition) {
                    mTab.setTextColor(getResources().getColor(R.color.tab_select_text_color));
                } else {
                    mTab.setTextColor(getResources().getColor(R.color.tab_normal_text_color));
                }
                ...
                if (textAllCaps) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
                        mTab.setAllCaps(true);
                    } else {
                        mTab.setText(mTab.getText().toString()
                                .toUpperCase(locale));
                    }
                }
            }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float lineLeft = currentTab.getLeft() + (defaultTabWidth - textWidth) / 2;
        float lineRight = currentTab.getRight() - (defaultTabWidth - textWidth) / 2;

        // if there is an offset, start interpolating left and right coordinates between current and next tab
        if (currentPositionOffset > 0f && currentPosition < tabCount - 1) {

            TextView nextTab = (TextView) tabsContainer.getChildAt(currentPosition + 1);
            float nextTextWidth = nextTab.getPaint().measureText(nextTab.getText().toString());
            ...
            final float nextTabLeft = nextTab.getLeft() + (defaultTabWidth - nextTextWidth) / 2;
            final float nextTabRight = nextTab.getRight() - (defaultTabWidth - nextTextWidth) / 2;
...
//indicatorHeight,值得就是指示条的高度,
        ...
        // draw underline
        //底部线
        rectPaint.setColor(getResources().getColor(R.color.Transparent));
